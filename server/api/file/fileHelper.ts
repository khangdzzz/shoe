...
export const appendData = (fileName: string, newItem: any): void => {
...
export const updateDataByNameAndStatus = (fileName: string, id: number): boolean => {
  const filePath = getFilePath(fileName);
  ensureFileExists(filePath);
...
export const findRecordByNameAndStatus = async (fileName: string, name: string): Promise<any | null> => {
  const filePath = getFilePath(fileName);
  ensureFileExists(filePath);
